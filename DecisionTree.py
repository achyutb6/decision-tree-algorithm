...
def classify(row, node):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        return list(node.predictions.keys())[0] #check
...
def print_tree(node, spacing=""):
    ...
    print (spacing + str(node.question)+ " " +str(node.depth) + " " + str(node.id))
...
def getInnerNodes(node, innerNodes =[]):
    ...
    if not isinstance(node, Decision_Node):
        return
    innerNodes.extend([node])
...
def computeAccuracy(rows, node):
    ...
    numAccurate = 0
    for row in rows:
        predictedClassLabel = classify(row, node)
        if (str(predictedClassLabel)==str(row[-1])):
            numAccurate+=1
